            ...
            <div class="md:grid md:grid-cols-3 md:gap-6">
                <div class="md:col-span-1">
                    <div class="px-4 sm:px-0">
                        <img src="{{ $event->image }}" alt="Workflow logo" />
                        <h3>{{ $event->name }}</h3>
...
@section('scripts')
    <script>
        ...
        const uploadUrl = '/event/' + '{{$event->slug}}' + '/upload';
        console.log(uploadUrl);
        uppyOne
            .use(DragDrop, { target: '.for-DragDrop' })
            .use(XHRUpload, {endpoint: uploadUrl,formData:true,fieldName:'files[]',headers:{'X-CSRF-TOKEN':"{{csrf_token()}}"}})
